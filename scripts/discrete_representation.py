...
def main() -> None:
    ...
    parser.add_argument('-p', '--num-envs', type=int, default=1, help='Number of environments used to collect the evaluation episodes.')
    ...
    cmd = f'python src/main.py hydra.run.dir=discr_repre/{datetime.now().strftime("%Y-%m-%d/%H-%M-%S")} '

    cmd += 'wandb.mode=online '
    cmd += f'wandb.name=discr_repre-{cfg.wandb.name} '
    cmd += f'wandb.group=discr_repre-{cfg.wandb.group} '
    ...
    cmd += 'collection.test.config.discr_repr=True '
